...
void initGame() {
    ...
    snake[0].cdel = 0;
    snake[0].rdel = 16;
    snake[0].col = (240 / 2) - (snake[0].width / 2);
    snake[0].row = (160 / 2);
 snake[0].aniState = SNAKEDOWN;
    ...
    for(int i = 1; i < 10; i++) {
        snake[i].width = 16;
        snake[i].height = 16;
        snake[i].col = (240 / 2) - (snake[0].width / 2);
        snake[i].row = (160 / 2) - (snake[0].height / 2) - (i * 16);
...
void updateGame() {
    ...
    shadowOAM[0].attr0 = (0<<8) | (0<<13) | (0<<14) | snake[0].row;
    shadowOAM[0].attr1 = (1<<14) | snake[0].col;
    shadowOAM[0].attr2 = ((0)<<12) | ((0)*32+(snake[0].aniState * 2));


    shadowOAM[1].attr0 = (0<<8) | (0<<13) | (0<<14) | apple.row;
    shadowOAM[1].attr1 = (1<<14) | apple.col;
    shadowOAM[1].attr2 = ((0)<<12) | ((0)*32+(4 * 2));


    for(int i = 1; i < 10; i++) {
        if (snake[i].isActive) {
            shadowOAM[i+1].attr0 = (0<<8) | (0<<13) | (0<<14) | snake[i].row;
            shadowOAM[i+1].attr1 = (1<<14) | snake[i].col;
            shadowOAM[i+1].attr2 = ((0)<<12) | ((0)*32+(snake[i].aniState * 2));
        }
    }
